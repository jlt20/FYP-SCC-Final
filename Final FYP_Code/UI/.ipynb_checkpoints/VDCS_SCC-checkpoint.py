...
def cal_VDCS_SCC(N_wind, i_GL, num_gen, num_bus, complex_voltages, Z_LUT, I_L, ppc):
    ...
    sum_ibr = np.zeros((num_bus, 1), dtype=complex)
    ...
    for i_w in range(N_wind):
        ...
        for i in range(1,N):
            ...
            for F in range(num_bus):
                Z_new[F][:] = Z_LUT[(i - 1), num_bus * F:num_bus * (F + 1)]
                ...
                for ii in range(1,10000):
                    ...
                    for m in range(len(i_GL)):
                        if ii == 1:   
                            ...
                            Ir = min(2, dc[m]*np.abs(dmagV_c)*alpha) # Icmax = 1? 
                            # Ir = min(1.2, dc[m]*np.abs(dmagV_c)*alpha) # Icmax = 1? 
                            Ia = np.real(I_L[i_GL[m]-1])
                            if Ia >= np.sqrt(2**2 - Ir**2):
                                Ia = np.sqrt(2**2 - Ir**2)
                            #if Ia >= np.sqrt(1.2**2 - Ir**2):
                            #    Ia = np.sqrt(1.2**2 - Ir**2)
                            Ifc[i_GL[m]-1] = Ia + 1j*Ir # Not sure if +1/-1 
                        sum_c[F] = sum_c[F] + Z_new[F][i_GL[m]-1] *(Ifc[i_GL[m]-1] - I_L[i_GL[m]-1])
                    I_sc[F][ii] = -(complex_voltages[F]+sum_c[F]) / Z_new[F][F]

                    for m in range(len(i_GL)):
                        sum_ibr[F] = 0
                        for n in range(len(i_GL)):
                            sum_ibr[F] = sum_ibr[F] + Z_new[i_GL[n]-1][i_GL[m]-1] * (Ifc[i_GL[n]-1] - I_L[i_GL[n]-1]) # Not sure about Ifc 
                        dV_c[F][m] = Z_new[F][i_GL[m]-1] * I_sc[F][ii] + sum_ibr[F]
    ...
    export_to_excel(np.abs(Isc), "Isc_test.xlsx")
